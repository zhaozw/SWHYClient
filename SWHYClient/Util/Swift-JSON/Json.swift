...
public class JSONClass {
    ...
    /// NSJSONSerialization
    public init(_ obj:AnyObject) { self._value = JSONClass.unwrap(obj) }
    /// pass the JSON object for another instance
    public init(_ json:JSONClass){ self._value = json._value }
}
/// class properties
extension JSONClass {
    ...
    public class func fromURL(url:String) -> JSONClass {
        return JSONClass(url:url)
    }
...
extension JSONClass {
    ...
    public subscript(idx:Int) -> JSONClass {
        switch _value {
        case let err as NSError:
            return self
        case let ary as NSArray:
            if 0 <= idx && idx < ary.count {
                return JSONClass(ary[idx])
            }
            return JSONClass(NSError(
    ...
    public subscript(key:String)->JSONClass {
        switch _value {
        case let err as NSError:
            return self
        case let dic as NSDictionary:
            if let val:AnyObject = dic[key] { return JSONClass(val) }
            return JSONClass(NSError(
                domain:"JSONErrorDomain", code:404, userInfo:[
                    NSLocalizedDescriptionKey:
                    "[\"\(key)\"] not found"
    ...
    public var asNull:NSNull? {
        return _value is NSNull ? JSONClass.null : nil
    }
    ...
    public var asArray:[JSONClass]? {
        switch _value {
        case let o as NSArray:
            var result = [JSONClass]()
            for v:AnyObject in o { result.append(JSONClass(v)) }
            return result
        default:
            return nil
        }
    ...
    /// with elements therein. nil otherwise
    public var asDictionary:[String:JSONClass]? {
        switch _value {
    ...
    public var allValues:JSONClass{
        if(self._value.allValues == nil) {
            return JSONClass([])
        }
        return JSONClass(self._value.allValues)
    }
    // gives all keys content in JSONClass object.
    public var allKeys:JSONClass{
        if(self._value.allKeys == nil) {
            return JSONClass([])
        }
        return JSONClass(self._value.allKeys)
    }
}
